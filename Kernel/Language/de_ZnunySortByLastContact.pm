...
sub Data {
    ...
    $Self->{Translation}->{"Defines the default ticket order (after priority sort) in the status view of the agent interface. Up: oldest on top. Down: latest on top."} = "Definiert die Standard-Ticketreihenfolge (nach Sortierung der Priorität) in der Statusansicht der Agentenschnittstelle. Up: Älteste oben. Unten: Neueste oben.";
    ...
    $Self->{Translation}->{"Ticket event module to update last customer contact timestamp after article create action."} = "Ticket-Event-Modul zum automatischen Setzen des Zeitstempels für den letzten Kundenkontakt nach Erstellung eines Artikels.";
